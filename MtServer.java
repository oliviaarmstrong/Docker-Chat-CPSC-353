...
public class MtServer {
  ...
  private void getConnection() {
    ...
    try {
      System.out.println("Waiting for client connections on port 9026.");
      ServerSocket serverSock = new ServerSocket(9026);
      // This is an infinite loop, the user will have to shut it down
      // using control-c
      while (true) {
        Socket connectionSock = serverSock.accept();
        Client myClient = new Client(connectionSock, "");
       
        // Add this socket to the list
        ClientList.add(myClient);
        // Send to ClientHandler the socket and arraylist of all sockets
        ClientHandler handler = new ClientHandler(myClient, this.ClientList);
